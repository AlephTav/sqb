...
func TestValuesStmt_ValuesAsCollectionOfCollections(t *testing.T) {
	...
	sqb.CheckParams(t, map[string]any{"p1": 1, "p2": 2, "p3": "a", "p4": "b", "p5": "c"}, st.Params())
}
...
func TestValuesStmt_ValuesAsMapOfCollections(t *testing.T) {
	...
	sqb.CheckParams(t, map[string]any{"p1": 1, "p2": 2, "p3": "a", "p4": "b"}, st.Params())
}
...
func TestValuesStmt_ValuesAsMapOfMaps(t *testing.T) {
	...
	st := NewValuesStmt(nil).
		Values(
			sqb.Map(
				"k1", sqb.Map("k1", 1, "k2", 2),
				"k2", sqb.Map("k1", "a", "k2", "b"),
			),
		)
...
func TestValuesStmt_AppendValues(t *testing.T) {
	...
	sqb.CheckSql(t, "VALUES (:p1), (:p2), (:p3, :p4), (:p5)", st.String())
	sqb.CheckParams(t, map[string]any{"p1": 1, "p2": "a", "p3": 1, "p4": 2, "p5": true}, st.Params())
}

func TestValuesStmt_ValuesAsQuery(t *testing.T) {
	st := NewValuesStmt(nil).
		Values("(1), (2)").
		Values(NewValuesStmt(nil).Values("(3)"))
...
func TestValuesStmt_OrderByColumn(t *testing.T) {
	st := NewValuesStmt(nil).
		Values("(1), (2), (3)").
		OrderBy("column1")
...
func TestValuesStmt_UnionOfValuesAndQueriesWithSorting(t *testing.T) {
	st := NewValuesStmt(nil).
		...
		Union(NewValuesStmt(nil).Values("('a'), ('b'), ('b')").OrderBy("column1", "DESC")).
		OrderBy("column1", "ASC")

	sqb.CheckSql(t, "(VALUES (1), (2), (3)) UNION (SELECT * FROM tb ORDER BY id ASC) UNION"+
		" (VALUES ('a'), ('b'), ('b') ORDER BY column1 DESC) ORDER BY column1 ASC", st.String())
	sqb.CheckParams(t, map[string]any{}, st.Params())
}
...
func TestValuesStmt_Copy(t *testing.T) {
	sqb.ResetParameterIndex()
	st := NewValuesStmt(nil).
		Values([]any{1}).
		OrderBy("column1", "DESC").
	...
	sqb.CheckSql(t, "VALUES (:p1) ORDER BY column1 DESC LIMIT 2 OFFSET 1", double.String())
	sqb.CheckParams(t, map[string]any{"p1": 1}, double.Params())
}

func TestValuesStmt_Clean(t *testing.T) {
	st := NewValuesStmt(nil).
		Values([]any{1}).
		OrderBy("column1", "DESC").
		Limit(2).
