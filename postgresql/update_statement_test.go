...
func TestUpdateStmt_Copy(t *testing.T) {
	sqb.ResetParameterIndex()
	st := NewUpdateStmt(nil).
		With("(SELECT * FROM t1)", "tb").
		OnlyTable("tb", "t").
	...
	sqb.CheckParams(t, map[string]any{"p1": "abc", "p2": 0}, double.Params())
}

func TestUpdateStmt_Clean(t *testing.T) {
	st := NewUpdateStmt(nil).
		With("(SELECT * FROM t1)", "tb").
		OnlyTable("tb", "t").
		Assign("c2", "abc").
