...
func TestDeleteStmt_WhereWithNestedConditionsAsClosure(t *testing.T) {
	st := NewDeleteStmt(nil).
		From("tb").
		Where("c1 IS NULL").
		Where(func(condition sql.ConditionalExpression) {
...
func TestDeleteStmt_ReturningColumnListWithAliases(t *testing.T) {
	st := NewDeleteStmt(nil).
		From("tb").
		Returning([]any{sqb.Map("a", "c1", "b", "c2", "c", "c3")})
